public class FightScene {
    ...
    public void fight(Person player, Person monster, Realm.FightCallback fightCallback) {
    ...
    private Boolean makeHit(Person defender, Person attacker, Realm.FightCallback fightCallback) {
        //Получаем силу удара
        int hit = attacker.attack();
        //Отнимаем количество урона из здоровья защищающегося
        int defenderHealth = defender.getHealthLevel() - hit;
        ...
        if (defenderHealth <= 0 && defender instanceof Player) {
            ...
            System.out.println(String.format("Враг повержен! Вы получаете %d опыт и %d золота", defender.getExperienceLevel(), defender.getGold()));
            attacker.setExperienceLevel(attacker.getExperienceLevel() + defender.getExperienceLevel());
